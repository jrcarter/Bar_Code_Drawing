...
package Bar_Code_Drawing is
   ...
   function New_Info (Width : in Positive; Height : in Positive; Scale : in Positive := 1) return Drawing_Info;
   ...
   -- For example,
   --    Declare a square Drawing_Info object
   ...
   -- Use a wide, rectangular drawing area and What.Code_128.Draw to create a Code-128 bar code in either or both representations
   -- instead
...
private -- Bar_Code_Drawing
   ...
   procedure Draw_Module (Info : in out Drawing_Info; X : in Natural; Y : in Natural; Line : in Boolean);
   ...
   -- Line indicates how to draw the module
   -- If Line, the module will stretch the entire height of Bitmap; otherwise the module is a square

   type Drawing_Info (Last_X : Positive; Last_Y : Positive) is tagged record -- Max coordinates of drawing area in pixels
      Width  : Positive;
      Height : Positive;
